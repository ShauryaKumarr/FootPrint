...
export default function ReceiptScanner() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div
        className="fc-card border-2 border-dashed border-[#E8E4DF] p-10 text-center mb-6 hover:border-[#0B3D2E]/30 transition-colors cursor-pointer"
      ...
      >
        ...
        <p className="text-[#0B3D2E] font-medium">
          {previewUrl ? 'Click to change image' : 'Click to upload a receipt image'}
        </p>
        <p className="text-sm text-[#4A5568] mt-1">JPEG, PNG, WebP, GIF — max 10 MB</p>
      </div>

      {image && items.length === 0 && !scanning && (
        <button
          onClick={handleScan}
          className="w-full fc-btn mb-6"
        >
          Scan Receipt
        </button>
      )}
      ...
      {items.length > 0 && (
        <div className="fc-card p-6 mb-6">
          ...
          <div className="space-y-3">
            {items.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-[#FFF5EB]">
                <div>
                  <p className="font-medium text-[#0B3D2E]">{item.name}</p>
                  <p className="text-sm text-[#4A5568]">{item.category}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-[#0B3D2E]">${item.price}</p>
                  <p className={`text-xs font-bold ${
                    item.grade === 'A' ? 'text-[#0B3D2E]' :
                    item.grade === 'B' ? 'text-[#0B3D2E]/80' :
          ...
          <div className="mt-5 pt-4 border-t border-[#E8E4DF]">
            {added ? (
              <p className="text-center text-[#0B3D2E] font-semibold">
              ...
              <div className="text-center">
                ...
                <button disabled className="px-6 py-2.5 bg-[#E8E4DF] text-[#4A5568] rounded-xl cursor-not-allowed font-medium">
                  Bank Not Connected
                </button>
              </div>
            ) : (
      ...
      {(items.length > 0 || error) && (
        <button
          onClick={reset}
          className="w-full fc-btn-outline"
        >
